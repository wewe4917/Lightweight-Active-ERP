...
def po_create(request):
    ...
    # 오늘 생성된 발주서 중 가장 큰(마지막) 번호를 찾음
    last_order = PurchaseOrder.objects.filter(po_number__startswith=prefix).aggregate(Max('po_number'))
    last_po_number = last_order['po_number__max']

    if last_po_number:
        # 마지막 번호가 있으면 끝 3자리를 잘라내서 1을 더함
        last_sequence = int(last_po_number.split('-')[-1])
        po_number = f'{prefix}{str(last_sequence + 1).zfill(3)}'
    else:
        # 오늘 생성된 발주서가 없으면 001번부터 시작
